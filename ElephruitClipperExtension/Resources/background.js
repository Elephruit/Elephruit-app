async function toggleClipperPanel(tab) {
  ...
  try {
    const response = await browser.tabs.sendMessage(tab.id, { type: "elephruit.panel.toggle.v5" });
  ...
  await browser.tabs.sendMessage(tab.id, { type: "elephruit.panel.toggle.v5" });
}
