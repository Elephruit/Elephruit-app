async function toggleClipperPanel(tab) {
  ...
  try {
    const response = await browser.tabs.sendMessage(tab.id, { type: "elephruit.panel.toggle.v3" });
  ...
  await browser.tabs.sendMessage(tab.id, { type: "elephruit.panel.toggle.v3" });
}
