...
/// The versioned JSON archive — Everything's complete, durable export format.
///
/// ### Contract
    ...
    public static func decoded(from data: Data) throws(AppError) -> ArchiveDocument {
        ...
        guard document.formatVersion <= currentFormatVersion else {
            throw .importFailed(
                format: "JSON archive",
                reason: """
                    This archive was written by a newer version of Everything \
                    (format \(document.formatVersion); this version reads up to \
                    \(currentFormatVersion)). Update Everything and try again.
                    """
            )
        }
