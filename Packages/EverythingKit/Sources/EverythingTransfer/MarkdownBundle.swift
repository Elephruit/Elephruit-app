...
///   everything-archive.json   ← the lossless companion
...
struct MarkdownBundleWriter {
    ...
    func write(to destination: URL) throws(AppError) -> ExportReport {
        ...
        do {
            ...
            try archiveData.write(
                to: destination.appending(path: "everything-archive.json", directoryHint: .notDirectory),
                options: [.atomic]
            )
            fileCount += 1
    ...
    private func makeReadme() -> String {
        ...
        - **`everything-archive.json`** — the complete, lossless record, including links,
          collection order, and saved searches. Import this file back into Everything to restore
