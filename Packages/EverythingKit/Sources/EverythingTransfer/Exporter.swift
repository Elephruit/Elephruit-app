...
public struct Exporter {
    ...
    public func buildArchive() throws(AppError) -> ArchiveDocument {
        ...
        let tags: [EverythingModel.Tag]
        let links: [ItemLink]
        let collections: [ItemCollection]
        let savedSearches: [SavedSearch]
        let attachments: [EverythingModel.Attachment]

        do {
            tags = try context.fetch(FetchDescriptor<EverythingModel.Tag>(sortBy: [SortDescriptor(\.slug)]))
            ...
            attachments = try context.fetch(FetchDescriptor<EverythingModel.Attachment>())
        } catch {
    ...
    static func record(for tag: EverythingModel.Tag) -> ArchiveTag {
        ArchiveTag(
            id: tag.id,
            name: tag.name,
            slug: tag.slug,
    ...
    static func record(for attachment: EverythingModel.Attachment) -> ArchiveAttachment {
        ArchiveAttachment(
            id: attachment.id,
