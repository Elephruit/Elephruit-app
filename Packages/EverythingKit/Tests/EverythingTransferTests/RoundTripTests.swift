...
struct BundleWritingTests {
    ...
    func writesJSONArchive() throws {
        ...
        let directory = URL.temporaryDirectory.appending(path: "EverythingExportTests/\(UUID().uuidString)", directoryHint: .isDirectory)
    ...
    func writesMarkdownBundle() throws {
        ...
        #expect(fileManager.fileExists(atPath: directory.appending(path: "everything-archive.json").path(percentEncoded: false)))
        ...
        let archiveData = try Data(contentsOf: directory.appending(path: "everything-archive.json"))
        let destination = try TransferFixture()
