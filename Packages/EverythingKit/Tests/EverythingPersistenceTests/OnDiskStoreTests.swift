...
struct OnDiskStoreTests {
    ...
    func everyEntityPersists() throws {
        ...
        do {
            ...
            let tag = EverythingModel.Tag(name: "sample")
            context.insert(tag)
            item.tags = [tag]
            ...
            context.insert(EverythingModel.Attachment(filename: "notes.pdf", typeIdentifier: "com.adobe.pdf"))
        ...
        #expect(try context.fetch(FetchDescriptor<EverythingModel.Tag>()).count == 1)
        ...
        #expect(try context.fetch(FetchDescriptor<EverythingModel.Attachment>()).count == 1)
    ...
    func backupIncludesCompanionFiles() throws {
        try withTemporaryStore { location, stack in
            ...
            #expect(contents.contains("Everything.store"))
            #expect(contents.contains { $0.hasPrefix("Everything.store") })
        }
    }
    ...
    func locationLayoutIsStable() {
        ...
        #expect(location.storeURL.lastPathComponent == "Everything.store")
