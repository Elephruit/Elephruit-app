...
struct SchemaComplianceTests {
    ...
    func everyEntityHasCompleteDefaults() throws {
        ...
        context.insert(Item())
        context.insert(EverythingModel.Tag(name: "untitled"))
        context.insert(ItemLink())
        context.insert(EverythingModel.Attachment())
        ...
        #expect(try fresh.fetch(FetchDescriptor<EverythingModel.Tag>()).count == 1)
        #expect(try fresh.fetch(FetchDescriptor<ItemLink>()).count == 1)
        #expect(try fresh.fetch(FetchDescriptor<EverythingModel.Attachment>()).count == 1)
    ...
    func schemaVersionIsReadable() {
        ...
        #expect(EverythingMigrationPlan.schemas.count == 1)
    }
