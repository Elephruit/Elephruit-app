...
/// A person's circle, addressed the way the Mac's call sites already address it — by colour name.
///
/// ### Why this is now a wrapper
/// It had its own monogram rule: the first letters of the first *two* words, so "Rosa María de la
/// Cruz" came out "RM" here and "RC" from ``ElephruitDesign/Avatar``, which is the same person
/// carrying two monograms on two screens — the exact fault `Avatar` was consolidated to end, left
/// in place because this type was written before it. There is one derivation now, and it is
/// `Avatar`'s: first word and last word.
///
/// What stays here is the argument this file's callers pass — a stored palette *name* rather than a
/// resolved `Color` — and the placeholder for a record with no name at all, which a monogram of ""
/// would otherwise draw as an empty circle.
///
/// Photographs are `Avatar`'s too, and this passes them straight through: see
/// ``ElephruitFeaturesCore/ContactPhotoStore`` for where the bytes come from and why the address
/// book is not read until a row is on screen.
struct PersonAvatar: View {
    let name: String
    let colorName: String?
    var size: CGFloat = Theme.Size.iconTileMedium
    var photo: Data?
