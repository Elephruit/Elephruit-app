    ...
    public var body: some View {
        LazyVGrid(
            ...
            if showsWeekdayHeader {
                // Indexed rather than keyed by the letter itself: half of them repeat — two S, two T
                // in English — and a `ForEach` keyed on the value would draw five columns.
            ...
            ForEach(0..<Self.leadingBlanks(of: month, calendar: calendar), id: \.self) { _ in
                Color.clear.frame(width: cellSize.width, height: cellSize.height)
            }

            ForEach(Self.days(of: month, calendar: calendar), id: \.self) { day in
                cell(day)
            }
        }
    }
