...
| **Contacts write** | *(same — Contacts has one access tier)* | *(same)* | **Off** | Per-edit confirmation in `ContactWriteBackSheet` | Refusal is reported; the local record keeps the edit either way | Five writable fields, enforced by `ContactWrite` having no others; `CNSaveRequest`, `store.execute`, and `CNMutableContact` asserted to appear once each; **never** creates or deletes |
